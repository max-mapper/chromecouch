var CouchData = function(couchRoot) {
  if(!couchRoot) couchRoot = "";
  
  var databases = {};

  function designDocs(database) {
    return couch.db(database, couchRoot).designDocs();
  }
  ...
  function readKey(db, ddoc, key) {
    var dfd = $.Deferred();
    couch.db(db, couchRoot).get(ddoc).done(function (data) {
      var obj = data;
      do {
          var nkey = key.shift();
          obj = obj[nkey];
  ...
  function fetchddoc(db, ddoc) {
    var dfd = $.Deferred();
    couch.db(db, couchRoot).get(ddoc).then(function (data) {
      var obj = {};
